        ...
        this.join = this.expr.extend({
            ...
            left: function (table, condition) {
                this._chain.push({
                    ...
                    table: this.dialect.expr.wrap(table),
                    condition: this.dialect.expr.wrap(table)        
                });
                return this;
            },
        ...
        this.insert = this.statement.extend({
            ...
            sqlite3: {
                toSQL: function (params) {
                    ...
                    for (var i = 0; i < this._values.length; i++) {
                        ...
                        if (_.isArray(values)) {
                            for (var j = 0; j < values.length; j++) {
                                if (aliases) {
                                    select.columns(this.dialect.alias(values[j], aliases[j]));
                                } else {
                            ...
                            for (var j = 0; j < columns.length; j++) {
                                if (aliases) {
                                    select.columns(this.dialect.alias(values[columns[j]], aliases[j]));
                                } else {
                    ...
                    return 'INSERT INTO '+this._table+' '+union.toSQL(params);
                }
            },
